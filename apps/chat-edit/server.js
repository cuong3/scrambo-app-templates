...
const STATIC_FILES = new Map([
  ...
  ["/command-chain.js", ["command-chain.js", "text/javascript; charset=utf-8"]],
  ["/session-lifecycle.js", ["session-lifecycle.js", "text/javascript; charset=utf-8"]],
  ["/chat-intent.js", ["chat-intent.js", "text/javascript; charset=utf-8"]],
...
export function createRequestHandler(options = {}) {
  const apiUrl = (options.apiUrl ?? process.env.SCRAMBO_API_URL ?? DEFAULT_API_URL).replace(/\/$/, "");
  ...
  function requireApiAccess() {
    if (!apiUrl) {
      throw new HttpError(503, {
        error: { code: "not_configured", message: "Scrambo API URL is not configured.", retryable: false },
      });
    }
    if (!apiToken) {
      throw new HttpError(503, {
  ...
  async function uploadFile(request, sessionId, url) {
    const name = url.searchParams.get("name")?.trim();
    const mimeType = url.searchParams.get("mimeType")?.trim() || "application/octet-stream";
    if (!name) {
      throw new HttpError(400, {
        error: { code: "missing_name", message: "Upload name is required", retryable: false },
      });
    }
    if (name.length > 255 || mimeType.length > 255 || /[\r\n]/.test(mimeType)) {
      throw new HttpError(400, {
        error: { code: "invalid_upload_metadata", message: "Upload metadata is invalid.", retryable: false },
      });
    }
    requireApiAccess();
    ...
    if (Number.isFinite(declaredLength) && declaredLength > maxUploadBytes) {
      throw new HttpError(413, {
        error: { code: "upload_too_large", message: `Upload exceeds the ${maxUploadBytes}-byte limit.`, retryable: false },
      });
    }
    ...
    activeUploads += 1;
    try {
      // Verify access to this session before writing the request body to the
      // server's temporary disk.
      await client.getSession(sessionId);

      const tempDirectory = await mkdtemp(join(tmpdir(), "scrambo-transcript-upload-"));
      const tempPath = join(tempDirectory, "asset.bin");
      ...
      try {
        ...
        const content = createReadStream(tempPath);
        const receipt = await client.uploadAssetContent(sessionId, asset.assetId, content, {
          mimeType,
          size,
        });
  ...
  return async function handle(request, response) {
    try {
      ...
      if (request.method === "POST" && match) {
        sendJson(response, 201, await uploadFile(request, match[1], url));
        return;
      }
      ...
      match = url.pathname.match(/^\/api\/sessions\/([^/]+)\/render-handoffs$/);
      if (request.method === "POST" && match) {
        sendJson(response, 202, await clientFor().createRenderHandoff(match[1], await readJson(request)));
        return;
      }

      match = url.pathname.match(/^\/api\/sessions\/([^/]+)\/render-handoffs\/([^/]+)$/);
      if (request.method === "GET" && match) {
        sendJson(response, 200, await clientFor().getRenderHandoff(match[1], match[2]));
        return;
      }
...
if (fileURLToPath(import.meta.url) === process.argv[1]) {
  const port = positiveInteger(process.env.PORT, 4174);
  const host = process.env.HOST?.trim() || "127.0.0.1";
  const server = createAppServer();
  server.listen(port, host, () => {
    console.log(`Scrambo Chat Edit is running at http://${host}:${port}`);
  });
